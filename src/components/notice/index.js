...
function getNoticeInstance (options) {
  noticeInstance = noticeInstance || Notification.newInstance({
    prefixCls: prefixCls,
    className: options.className,
    styles: options.wrapStyles || {top: `${top}px`, right: 0}
  })
...
function notice (type, options) {
  ...
  const transitionName = options.transitionName || 'move-notice-right'
  const duration = (options.duration === 0) ? 0 : options.duration || defaultDuration
  const noticeName = options.noticeName || ''
  const icon = options.icon || ''
  name++
  let instance = getNoticeInstance(options)
  let content
  let withIcon
  ...
  if (type == 'normal') {
    ...
    const outlineIcon = with_desc === '' ? '' : (icon ? '' : '-outline')
    withIcon = true
    content = `
